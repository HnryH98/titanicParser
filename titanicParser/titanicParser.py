...
def getPassengerNames(splitLine):
    ...
    surname = splitLine[3]
    surname = surname.replace('"','')#remove speechmark 
     
    otherNames = splitLine[4]      
    otherNames = otherNames.replace('"','')#remove speechmark 
    ...
    if len(splitOtherNames)>=4:#if not length of 4, middle name is not present in data
        ...
        #middle name must not be available if birth name is present at position 3 in list:
        #(birth name indicated by presence of opening bracket)
        if '(' in middleName:
...
def mainFunc():
    ...
    with open('titanic_output.csv.csv', mode='w', newline='',encoding='utf-8') as outputFile:
    ...
    for line in fileContent:
        ...
        with open('titanic_output.csv.csv', 'a', newline='', encoding='utf-8') as outputFile:
            outputWriter = csv.writer(outputFile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
